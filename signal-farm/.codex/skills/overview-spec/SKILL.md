...
Before drafting from memory, fetch the canonical tx scaffold for the target overview and use that as the minimum valid schema. Run `tx doc add overview <name> --title "<title>"` if the doc does not exist yet, then run `tx doc show <name> --md`. That output is the baseline shape to preserve. Add structure on top of it; do not invent frontmatter fields or remove tx-required top-level sections.
...
- Preserve all material information before summarizing. Carry distinctions in scope, stakeholder expectations, dependency notes, risks, success metrics, and historical constraints into the tx-managed doc explicitly.
- If the source material needs more structure than the scaffold provides, add new headers, subheaders, tables, or appendices rather than compressing detail into broad summary paragraphs. The tx scaffold is the minimum schema, not the maximum shape.
...
Read [references/octospark-overview-example.md](references/octospark-overview-example.md) before drafting when you need a realistic overview example derived from a long-form requirements document.

That example is useful because it shows:

- project-level context that is broader than a single PRD
- launch scope, actors, platform targets, and governance decisions in one place
- how to preserve business and product context before splitting it into smaller companion PRDs and design docs
...
Immediately after scaffolding or opening an existing overview, fetch the canonical markdown shape:

```bash
tx doc show $ARGUMENTS --md
```

Treat that output as the minimal schema for the document. Keep the required frontmatter keys and required top-level sections intact, and add new headings whenever they are needed to preserve source detail.
...
The scaffold returned by `tx doc show <name> --md` is the minimum acceptable schema. You may add new headers, subheaders, tables, appendices, diagrams, and migration-specific sections whenever they help preserve information from the source material. Prefer explicit structure over silent compression.
...
Re-read the plan section. For each item in the plan, confirm it appears in at least one subsequent section. Confirm every meaningful source section or source fact has been preserved, relocated, or explicitly superseded. No silent drops. No stubs, no empty tables, no "..." placeholders. Verify plan file exists at the path in frontmatter and its content is consistent with the doc.
