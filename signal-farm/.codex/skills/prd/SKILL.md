...
Before drafting from memory, fetch the canonical tx scaffold for the target PRD and use that as the minimum valid schema. Run `tx doc add prd <name> --title "<title>"` if the doc does not exist yet, then run `tx doc show <name> --md`. That output is the baseline shape to preserve. Add structure on top of it; do not invent frontmatter fields or remove tx-required top-level sections.
...
- When migrating or splitting an existing spec, preserve all material information before summarizing. Carry thresholds, exception cases, dependencies, examples, stakeholder distinctions, and open questions across explicitly.
- If source content does not fit the default headings cleanly, add new headers, subheaders, tables, or appendices rather than compressing multiple ideas into generic prose. The tx scaffold is the minimum schema, not the maximum shape.
...
Read [references/contextual-prompt-bandit-prd-example.md](references/contextual-prompt-bandit-prd-example.md) before drafting when you want a concrete tx-native PRD example.

That example is useful because it shows:

- clean separation of problem, scope, personas, requirements, acceptance criteria, and non-goals
- EARS requirements written in parser-friendly YAML with stable IDs
- a companion relationship with a subsystem design without leaking implementation detail into the PRD
- concise but specific non-functional requirements that still leave room for the design doc to own the HOW
...
Immediately after scaffolding or opening an existing PRD, fetch the canonical markdown shape:

```bash
tx doc show $ARGUMENTS --md
```

Treat that output as the minimal schema for the document. Keep the required frontmatter keys and required top-level sections intact, and layer additional headings on top when the source material needs more structure.
...
The scaffold returned by `tx doc show <name> --md` is the minimum acceptable schema. You may add new headers, subheaders, tables, appendices, and migration-specific sections whenever they help preserve information from the source material. Prefer extra structure over silently dropping nuance.
...
- Migration coverage: every meaningful source section or source fact has been preserved, relocated, or explicitly superseded. No silent drops.
